...
class TestLinear(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_activation_fn.init_sample.side_effect = lambda: 0.01
    ...
    def test_initialization_weights_content(self):
        ...
        expected_weights = np.array([
            [0.01] * self.linear_layer.weights.shape[1],
            [0.01] * self.linear_layer.weights.shape[1],
            [0.01] * self.linear_layer.weights.shape[1],
        ], dtype=float)
    ...
    def test_forward_pass_multiplication_and_activation(self):
        ...
        # we multiply [1...5] by [[[0.01] * 5] * 3]
        expected_raw_output = np.array([.15, .15, .15], dtype=float)
